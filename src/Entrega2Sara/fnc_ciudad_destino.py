...
def fnc_ciudad_destino(hormiga:int,
                   M:np.ndarray,
                   ciudad_inicial:int,
                   n:int,
                   Tao:np.ndarray,
    ...
    probabilidad:np.ndarray = np.zeros(n) #np.zeros((1,n))
    denominador:float = 0.0
    for i in range(n): #bucle para calcular la sumatoria
    #    verificar matriz de ciudades visitadas y que i no sea la ciuidad de inicio del arco.
        if  M[hormiga,i] == 0 and i != ciudad_inicial:
